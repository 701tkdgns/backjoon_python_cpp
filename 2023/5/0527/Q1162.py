...
#     while hq:
#         cnt, node, dist = heapq.heappop(hq)
#         if dp[node][cnt] < dist:
#             continue
#         for new_node, new_dist in lst[node]:
#             if dist + new_dist < dp[new_node][cnt]:
#                 dp[new_node][cnt] = dist + new_dist
#                 heapq.heappush(hq, [cnt, new_node, new_dist + dist])
#             if cnt < K and dp[new_node][cnt + 1] > dist:
#                 dp[new_node][cnt + 1] = dist
#                 heapq.heappush(hq, [cnt + 1, new_node, dist])
#
#
...
# https://wookcode.tistory.com/156
